...
- Model  
   ...
   ```python
   # Only word, except josa, norm(normalization) : 표현 방법이 다른 단어들을 통합시켜서 같은 단어로 만들어줌, stem(stemming) : 어간 추출
   def tokenize(doc):
   ...
   ```
   텍스트 파일을 불러온 후, 텍스트 데이터(영화 리뷰)에 대해 형태소 단위의 Tokenizing을 진행한다. Tokenizing 결과값과 긍정/부정 데이터를 data_set 리스트에 저장한다.
